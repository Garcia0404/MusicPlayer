...
export const RandomBtn = () => {
  const { random,setRandom } = useContext(contexto)
  const change = random?'block':'hidden'
  const changeIcon = random?'fill-greenMain':'fill-white9'
  function handleClick (){
    setRandom(!random)
  }
  return (
    <span onClick={handleClick} className='hidden mobileLg:block active:scale-75 transition-all relative'>
      <svg className={`${changeIcon} fill-white w-5 h-5 hover:scale-110 cursor-pointer transition-all`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
