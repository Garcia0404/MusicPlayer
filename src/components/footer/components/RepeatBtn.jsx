...
export const RepeatBtn = () => {
  const { repeat,setRepeat } = useContext(contexto)
  const change = repeat?'block':'hidden'
  const changeIcon = repeat?'fill-greenMain':'fill-white9'
  function handleClick(){
    setRepeat(!repeat)
  }
  return (
    <span onClick={handleClick} className='hidden mobileLg:block relative active:scale-75 transition-all'>
      <svg className='fill-white9 w-5 h-5 hover:scale-110 cursor-pointer transition-all' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
